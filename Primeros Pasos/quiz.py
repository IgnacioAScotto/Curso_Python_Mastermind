titulo = 'Bienvenido al Test para saber cuanto te gusta el queso'
print('\n'+ titulo + '\n' + '-' + '\n')

puntuacion = 0

opcion =  input('Pregunta 1: Que haces cuando ves una tabla de quesos?\n'
                'A - Salgo corriendo\n'
                'B - Pruebo uno o algunos de los quesos\n'
                'C - No puedo evitar devorarla\n'
                'Tu respuesta: ')

if opcion == 'A':
    puntuacion += 0
elif opcion == 'B':
    puntuacion += 5
elif opcion == 'C':
    puntuacion += 10
else:
    print('\nLas opciones posibles son: A, B y C')
    exit()


opcion = input('Pregunta 2: Como te gusta la hamburguesa?\n'
               'A - Sin queso\n'
               'B - Con queso\n'
               'C - Pan y queso\n')

if opcion == 'A':
    puntuacion += 0
elif opcion == 'B':
    puntuacion += 5
elif opcion == 'C':
    puntuacion += 10
else:
    print('\nLas opciones posibles son: A, B y C')
    exit()



opcion = input('Pregunta 3: Sos intolerable a la lactosa?\n'
               'A - Si\n'
               'B - A veces\n'
               'C - No\n')

if opcion == 'A':
    puntuacion += 0
elif opcion == 'B':
    puntuacion += 5
elif opcion == 'C':
    puntuacion += 10
else:
    print('\nLas opciones posibles son: A, B y C')
    exit()


print(f'\nTu puntuacion hasta ahora es: {puntuacion}')